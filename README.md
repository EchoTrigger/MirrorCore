...
为降低重复内容、提升阅读效率，我们将 `docs/GETTING_STARTED.md` 与 `docs/DEVELOPMENT.md` 的关键点合并为以下要点：
...
为降低重复与阅读成本，项目文档以“主 README 汇总”为中心：

- 主干内容（愿景、架构、MCP 代码模式、快速开始、示例）统一在 `README.md`。
- 将 `docs/GETTING_STARTED.md` 与 `docs/DEVELOPMENT.md` 的重合内容合并并精简至 README，避免重复。
- `docs/API.md` 用于接口详细规格；README 提供概览与入口。
- 文档拆分以“面向读者任务”为准（例如：开发者查看 README 与 API；部署手册在商业版本仓库）。

如需立即清理，请告知希望保留的文件清单，我们将按你的偏好进行删除与归档，并在 README 中补齐必要链接。
...
本仓库当前阶段以“功能验证”为主，重点实现了以下后端能力，供开发者联调与扩展：
...
说明：为减少阅读成本，“快速入门”和“开发指南”的关键内容合并至本 README。
